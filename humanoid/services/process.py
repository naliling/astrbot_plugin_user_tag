...
from ..slots import Slot, is_meal_event, is_sleep_event
...
# 工作类的说法太杂，单靠「工作/会议」两个词会漏掉「跟客户过方案」「改海报」这种
# 真在上班的时段。放在最后一级，具体的类别先抢。
_WORK_WORDS = (
    "工作", "办公", "事务", "处理", "会议", "开会", "方案", "客户", "汇报", "对接",
    "评审", "面试", "值班", "加班", "材料", "报表", "沟通", "写", "改", "弄", "赶",
    "投", "回复", "对稿", "排练", "备课", "论文", "代码", "订单", "业绩",
)
...
class ProcessService:
    ...
    def _push_history(self, proc: dict) -> None:
        """一个过程结束时记一笔，让「刚做完什么」能被后续上下文引用。"""
        limit = int(self.config.process_history_limit)
        name = str(proc.get("name", "")).strip()
        if limit <= 0 or not name:
            return
        history = self._scope.get_self("process_history")
        if not isinstance(history, list):
            history = []
        if history and history[-1] == name:
            return
        history.append(name)
        self._scope.set_self("process_history", history[-limit:])
    ...
    def recent(self, limit: int | None = None) -> list[str]:
        ...
        if not isinstance(history, list) or not history:
            return []
        cap = int(self.config.process_history_limit) if limit is None else limit
        if cap <= 0:
            return []
        return [str(item) for item in history[-cap:]][::-1]
    ...
    def _generate_process_for_slot(self, now: datetime, slot: Slot) -> dict:
        ...
        return {
            # 过程的名字直接用日程里那件事：以前从模板里抽一个「去一个地方」，
            # 与同一份注入里日程说的「跟客户过方案」互相打脸，模型只能挑一个信。
            "name": str(slot.get("event") or "").strip() or str(template.get("name") or "随意活动"),
            "category": category,
    ...
    def _infer_category(self, slot: Slot) -> str:
        """日程事件 → 生活类别。决定「手上在做的」下面挂哪一串阶段。

        以前用一堆单字关键字判（其中「早/午/晚」会把「晚间洗漱」判成用餐，
        「跟客户过方案」因为不含「会议」而落到自由时间，于是阶段写着「暂时不安排目标」）。
        睡眠与用餐直接用 slots 里那一套（吃饭优先于午睡），工作类放到最后兜底，
        让更具体的类别先抢。
        """
        event = str(slot.get("event", "")).lower()
        if is_meal_event(event):
            return "用餐"
        if is_sleep_event(event):
            return "睡眠"
        if any(k in event for k in ("学习", "读书", "复习", "做题", "上课", "作业", "背", "考试")):
            return "学习"
        if any(k in event for k in ("运动", "跑步", "健身", "锻炼", "打球", "游泳", "瑜伽", "拉伸", "散步")):
            return "运动"
        if any(k in event for k in ("家务", "整理", "打扫", "做饭", "洗衣", "拖地", "收东西", "买菜")):
            return "家务"
        if any(k in event for k in ("创作", "写作", "画", "设计", "手工", "剪视频", "拍")):
            return "创作"
        if any(k in event for k in ("社交", "聚会", "聊天", "朋友", "吃饭", "约会", "同事")):
            return "社交"
        if any(k in event for k in ("通勤", "路上", "出行", "车", "地铁", "公交", "高铁", "飞机", "取快递")):
            return "通勤"
        if any(k in event for k in _WORK_WORDS):
            return "工作"
        return "自由时间"

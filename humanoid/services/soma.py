...
STEP_MINUTES = 10.0
SLEEP_KEYWORDS = ("睡", "眠", "午休", "小憩")
MEAL_KEYWORDS = ("餐", "吃", "饭", "外卖", "食堂")

SLEEP_PRESSURE_PER_HOUR = 7.5      # 清醒时每小时攒的睡眠压力
SLEEP_RELIEF_PER_HOUR = 16.0       # 睡着时每小时消解的睡眠压力
...
class SomaService:
    ...
    def is_sleep_time(self, moment: datetime) -> bool:
        ...
        if slots:
            ...
            return any(k in event for k in SLEEP_KEYWORDS)
        cfg = self.config
    ...
    def is_eating_time(self, moment: datetime) -> bool:
        event = str(self.slot_at(moment).get("event", ""))
        return any(k in event for k in MEAL_KEYWORDS)
    ...
    def advance(self, now: float | None = None) -> dict[str, float]:
        ...
        for index in range(steps):
            ...
            self._integrate(step_seconds / 60.0, point, index == steps - 1)
            moment = point
    ...
    def _integrate(self, minutes: float, moment: datetime, is_last: bool) -> None:
        cfg = self.config
        hours = minutes / 60.0
        ...
        if asleep:
            self._set("sleep_pressure", self._get("sleep_pressure", 20.0) - SLEEP_RELIEF_PER_HOUR * hours)
            ...
            if in_night:
                # 该睡的时候醒着 = 在欠睡。封顶 SLEEP_DEBT_CAP：欠到一定程度之后多欠的
                # 部分已经反映在困意里，再堆只会让后面的系数失去区分度。
                debt = min(SLEEP_DEBT_CAP, self._get("sleep_debt", 0.0) + hours)
        ...
        if is_last:
            self.data["last_state_hour"] = int(moment.hour)

    def _record_wake(self, moment: datetime) -> None:
        """一觉醒来：按实际睡了多少清掉睡眠压力。

        这里**不记睡眠债**。债完全由「生物钟夜里醒着多久 / 睡着多久」对称积分出来。
        之前在这里用「睡够没够 sleep_need_hours」补一笔债，会撞上一个真实现象：
        ...
        if slept_hours >= 1.5:
            # 小干不会把压力清干净：睡得越短，醒来时残留的困意越多。
            relief = min(12.0, max(0.0, slept_hours * 1.5))
            self.data["sleep_pressure"] = _rounded(max(0.0, self._get("sleep_pressure", 20.0) - relief))
    ...
    def circadian_dip(self, hour_of_day: float) -> float:
        """昼夜低谷强度 0~1：凌晨最低，午后有个次低谷，傍晚最清醒。"""
        cfg = self.config
        anchor = (cfg.night_start_hour + cfg.night_end_hour) / 2.0

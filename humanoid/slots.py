...
# 日程里算「她在睡」的事件名关键词。不能只用「觉」：「觉得」会把一句普通话误判成睡着。
SLEEP_WORDS = ("睡", "眠", "午休", "小憩", "懒觉", "入睡")
# 吃饭优先于「午休」：「午餐与午休发呆」是在吃饭，不是在睡觉。
MEAL_WORDS = ("餐", "吃", "饭", "外卖", "食堂")


def is_sleep_event(event: Any) -> bool:
    text = str(event or "")
...
def is_meal_event(event: Any) -> bool:
    text = str(event or "")
